...
class Arm_2D_v3(gym.Env):
    ...
    def __init__(self):
            ...
            self.PnP_action = 0
            self.target_placed = 0                          #bool to check if the target is placed at the goal or not
            ...
            self.target_tray = self.remaining_targets
    ...
    # def sort_by_dist(self, all_target_pos):
    #     distances_from_goal = []
    #     for target in all_target_pos:
    #         distances_from_goal.append(euclidean([200, 0], target))
    #     sorted_targets = [pos for _, pos in sorted(zip(distances_from_goal, all_target_pos))]
    ...
    def Update_target_after_pick(self):
        self.target_tray = self.all_target_pos
    ...
    #     if self.PnP_action == 0 and self.target_placed == 1:    #target is placed at the goal
    #         self.target
    ...
    def step(self, action):
        ...
        self.tip_pos = [P[-1][0,3], P[-1][1,3]]                          #calculating tip position
        ...
        dis_err_target = euclidean(self.target_pos, self.tip_pos)          #distance between target and tip
        dis_err_goal = euclidean([200, 0], self.tip_pos)                   #distance between tip and goal

        # The arm is looking for the target, PnP_action is 0 now
        if self.PnP_action == 0:
            goal_tip_dis_x = 0      # if no object is picked, distance between goal and tip is not calculated
            goal_tip_dis_y = 0
            ...
            if (dis_err_target > -close_enough_tar and dis_err_target < close_enough_tar):      # If reach to target, give large reward, set PnP_action to 1
                print("Pick target!!!")              #current target is picked up 
                reward = 10
                #Update
                self.PnP_action = 1
                self.Update_target_after_pick()
                # time.sleep(0.1)

            if (abs(self.theta[0] - self.theta_last[0]) < 0.001 and abs(self.theta[1] - self.theta_last[1]) < 0.001):    #punishment for idling ?
                # print('stay!!!!!!!!!!!!!!!!')
                reward = -1

            self.current_error = dis_err_target         # Update current error, current_error tracks the distance between target and tip
        ...
        if self.current_score <= -20 or self.current_score >= 100:     # Finish an epoch if accumulative reward is too small
            done = True
        else:
            done = False
    ...
    def reset(self):
        ...
        if self.PnP_action == 1 and self.target_placed == 0:             #if the target did not reach the goal, reload the target
            self.all_target_pos = self.target_tray                       #new set of target positions are generated
            self.target_pos, self.remaining_targets = self.choose_target(self.all_target_pos)
